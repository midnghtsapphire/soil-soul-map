...
const ExploreMap: React.FC<ExploreMapProps> = ({ listings, className }) => {
    ...
    listingsData.forEach((listing) => {
      ...
      const popup = new mapboxgl.Popup({
        ...
        <div style="font-family: system-ui, sans-serif; padding: 8px;">
          <h3 style="font-size: 16px; font-weight: 600; margin-bottom: 6px; color: #1a1a1a;">
            ${listing.name}
          </h3>
          ...
          <p style="font-size: 13px; color: #444; margin-bottom: 12px; line-height: 1.4; display: -webkit-box; -webkit-line-clamp: 2; -webkit-box-orient: vertical; overflow: hidden;">
            ${listing.description}
          </p>
          <a href="/listing/${listing.id}" style="display: inline-block; font-size: 13px; color: #4A7C59; font-weight: 500; text-decoration: none;">
            View Details →
          </a>
        </div>
      `);
  ...
  if (!mapboxToken) {
    return (
      <div className={`relative rounded-xl overflow-hidden bg-card border border-border ${className}`}>
        ...
        <div className="relative z-10 flex flex-col items-center justify-center h-full p-8 text-center min-h-[500px]">
          ...
          <p className="text-muted-foreground mb-6 max-w-sm">
            Enter your Mapbox public token to view listings on a map.
  ...
  return (
    <div className={`relative rounded-xl overflow-hidden shadow-lg ${className}`}>
      ...
      <div className="absolute bottom-4 left-4 right-4 flex justify-center gap-6 p-3 rounded-xl bg-card/90 backdrop-blur-sm border border-border/50">
        ...
        <div className="flex items-center gap-2 text-sm">
          <div className="w-3 h-3 rounded-full" style={{ backgroundColor: '#C4A77D' }} />
          <span className="text-muted-foreground">Restaurants</span>
        </div>
      </div>

      {/* Results count */}
      <div className="absolute top-4 left-4 px-3 py-1.5 rounded-lg bg-card/90 backdrop-blur-sm border border-border/50 text-sm">
